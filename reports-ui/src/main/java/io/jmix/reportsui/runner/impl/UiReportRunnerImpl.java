...
public class UiReportRunnerImpl implements UiReportRunner {
    ...
    @Override
    public void multiRun(UiReportRunContext context, String multiParamAlias, Collection multiParamValue) {
        prepareContext(context);
        if (needToShowParamsDialog(context)) {
    ...
    protected void openReportParamsDialog(UiReportRunContext context, @Nullable ReportInputParameter inputParameter,
        ...
        inputParametersDialog.setRunInBackgroundMode(context.getRunInBackgroundMode());
    ...
    protected boolean needToRunInBackground(UiReportRunContext context) {
        RunInBackgroundMode mode = context.getRunInBackgroundMode();
        if (mode == RunInBackgroundMode.YES) {
            return true;
        }

        if (mode == null || mode == RunInBackgroundMode.DEFAULT) {
            return reportsClientProperties.getUseBackgroundReportProcessing();
        }
        return false;
    }

    protected boolean needToShowParamsDialog(UiReportRunContext uiReportRunContext) {
        ShowParametersDialogMode mode = uiReportRunContext.getShowParametersDialogMode();
        Report report = uiReportRunContext.getReport();
        if (mode == ShowParametersDialogMode.YES) {
            return true;
        }
        if (mode == null || mode == ShowParametersDialogMode.IF_REQUIRED) {
